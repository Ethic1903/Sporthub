...
| Frontend console | `:4173` (host) | Nginx со статичным UI поверх GraphQL |
...
## Frontend console (GraphQL)

Лёгкий операторский интерфейс лежит в папке `frontend/` и полностью работает поверх GraphQL-эндпоинта, который отдаёт `api-gateway`. В нём есть формы регистрации/логина, фильтры площадок, создание брони, подтверждение/отмена и активити-лог. Теперь происходящее упаковано в Nginx-контейнер.
...
Команда выполнит `docker compose up -d --build frontend`, соберёт образ из `frontend/Dockerfile` и поднимет контейнер на `http://localhost:4173`. Тот раздаёт статические файлы (`index.html`, `styles.css`, `app.js`) и публикует конфигурационный скрипт `/__sporthub_config__.js`, в котором прокидывается URL GraphQL-шлюза.

- `FRONTEND_GATEWAY_URL` — переменная окружения контейнера (значение подставляется в `__sporthub_config__.js`). В docker-compose по умолчанию установлен `http://localhost:8080/graphql`, чтобы браузер на хосте бил ровно в проброшенный порт gateway. При разворачивании в другой среде переменную можно переопределить.
- При необходимости адрес можно переопределить прямо в браузере через `localStorage.setItem("gatewayUrl", "http://host:port/graphql")` — `app.js` сначала проверяет сохранённое значение, потом значение из конфиг-скрипта, и уже затем падает обратно на `http://localhost:8080/graphql`.

`make compose-up` также соберёт и запустит `frontend`, так что UI автоматически появится вместе со всеми сервисами.

### Возможности UI

- **Auth**: регистрация и вход, токен кладётся в `localStorage` и автоматически добавляется в заголовок Authorization.
- **Facilities**: фильтр по городу и типу, карточки площадок с быстрым копированием `facilityId`.
- **Booking**: форма создания (есть кнопка демо-префилла), список броней с кнопками подтверждения/отмены.
- **Monitoring**: статус соединения с gateway и активити-лог всех операций.
